...
Далее нужно прокинуть публичные ключи на сервера и добавить приватные, которые находятся *home/deploy_user/.ssh/id_rsa_stage* и *home/deploy_user/.ssh/id_rsa_prod*, в GitLab Variables, чтобы Runner мог к ним подключаться.
```bash
ansible-playbook -i inventory/hosts.ini playbooks/setup-deploy-servers.yml --ask-vault-pass
```
После завершения плейбука добавьте приватные ключи (`id_rsa_stage` и `id_rsa_prod`) в **Settings > CI/CD > Variables**.
*   **Type**: `File`
...
На этом настройка закончена, осталось только написать .gitlab-ci.yml. После создания ветки добавить ее в protected branches, чтобы ветка видела переменные 
